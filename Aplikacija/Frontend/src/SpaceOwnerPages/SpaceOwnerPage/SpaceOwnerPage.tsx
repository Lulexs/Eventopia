...
export default function SpaceOwnerPage(props: OrganizerPageProps) {
  ...
  const updateUserForm = useForm({
    ...
    validate: {
      firstName: (value) => (value.length > 0 ? null : "Empty first name field"),
      lastName: (value) => (value.length > 0 ? null : "Empty last name field"),
      city: (value) => (value != null && value.length > 0 ? null : "Empty city field"),
      address: (value) => (value != null && value.length > 0 ? null : "Empty address field"),
      phoneNumber: (value) => (value.length > 0 ? null : "Empty phone number field"),
      currentPassword: (value) => (value.length > 0 ? null : "Empty current password field"),
      newPassword: (value) => (value.length > 0 ? null : "Empty new password field"),
    },
  });
  ...
  const respondToReservation = async (reservationId: number, response : string) => {
    try {
      await axios.put(
        `${import.meta.env.VITE_DB_SERVER}/Space/respondToSpaceReservation/${reservationId}/${response}`
      );
  ...
  const newPasswordProps = updateUserForm.getInputProps("newPassword");
    ...
    >
      <Flex className={classes.contentContainerFlex}>
        ...
        <Stack className={classes.contentStack}>
          ...
          >
            <Stack w="50%">
              ...
              <PasswordStrength label="New password" 
                                ...
                                key={updateUserForm.key("newPassword")}
              ...
              >
                ...
                <Button type="submit"
                  onClick={async (event) => {
                    ...
                    console.log(values);
                    await axios
                      .put(`${import.meta.env.VITE_DB_SERVER}/Account/updateUser`, {
                        ...values
                      })
                      .then((resp) => {
          ...
          >
            <StatsCard title="Rentable spaces" current={isStatisticsLoading || statisticsError ? 0 : statistics?.rentableSpaces || 0} />
            <StatsCard title="Total rents" current={isStatisticsLoading || statisticsError ? 0 : statistics?.totalRents || 0} />
          </Fieldset>
        </Stack>
      </Flex>
      <Flex className={classes.contentContainerFlex}>
          ...
          reservations?.map((reservation, idx) => (
            ...
            >
              ...
              {reservation.status == "WaitingConfirmation" && (
                <Group>
                  <Button bg="green" fullWidth onClick={() => respondToReservation(reservation.id, "accept")}>
                    Accept
                  </Button>
                  <Button bg="red" fullWidth onClick={() => respondToReservation(reservation.id, "reject")}>
                    Reject
                  </Button>
                </Group>
              )}
